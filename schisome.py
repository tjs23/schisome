...
class SchisomeDataSet(BaseDataSet):
  ...
  def add_analysis_tracks(self, recon_label='recon_all', latent_label='latent_all', pred_label='class_pred_all',
                          marker_label='training', split_idx=None, max_missing=0.35):

    # Input data
    klass_labels  = self.get_marker_labels(marker_label)
    train_klasses = self.get_marker_data(marker_label)
    ...
    pred_score_ensemble = self.get_pred_class_data(pred_label)    
    ...
    pred_klass_labels = self.get_pred_class_labels(pred_label)
    ...
    new_profiles =  np.array(zfill_profiles)
    new_profiles[invalid,:] = 0.0
    self.set_profile_data('zfill', new_profiles)
